...
namespace hva{
    ...
    void NewVulkanApp::loadModels() {
        ...
        uboVP.P = glm::perspective(glm::radians(30.0f), (float) vulkanSwapChain->width()/(float)vulkanSwapChain->height(),
                                 0.1f, 100.0f);
        ...
        test.transform(glm::rotate(glm::mat4(0.5f),20.0f,glm::vec3(1.0f,1.0f,1.0f)));
    ...
    Node NewVulkanApp::subdivideNode(Node sourceNode) {
        Node node1, node2, node3, node4, node5, outNode, tempNode;
        node1 = sourceNode;
        node2 = sourceNode;
        node3 = sourceNode;
        node4 = sourceNode;
        ...
        M =  glm::translate(glm::mat4(1.0f), glm::vec3(0.0f,0.0f,1.0));
        tempNode.transform(M);
        outNode.addChild(tempNode);

        //outNode.transform(glm::mat4(0.75f));
        //outNode.addChild(node5);
    ...
    void NewVulkanApp::updateModels() {
        ...
        uboVP.lightPos = glm::vec4((float)-0.8f,(float)1.8f,3.2f,1.0f);
